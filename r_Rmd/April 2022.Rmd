...
```{r d18-table, include = TRUE}
p_load(gt, gtExtras)

smoking_2019 <- smoking %>% 
  select(cou, time, group, percent_pop) %>% 
  filter(time == 2019)
...
smoking %>% 
  ...
  left_join(smk_19_w) %>% 
  gt() %>% 
  gt_sparkline(smk_data)
  pivot_wider(names_from = group, values_from = percent_pop) %>% 
  arrange(-Total) %>% 
  group_by(cou) %>% 
  summarize(smoking_data = list())
  
  gt_sparkline_tab <- mtcars %>%
   dplyr::group_by(cyl) %>%
   # must end up with list of data for each row in the input dataframe
   dplyr::summarize(mpg_data = list(mpg), .groups = "drop") %>% view()
   gt() %>%
