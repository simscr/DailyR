...
```{r comp-plot, include = FALSE}

ncaa_sum %>% 
  filter(classification_name == "NCAA Division I-FBS") %>% 
  ggplot() + 
  geom_linerange(aes(x = sports, 
                   ymin = exp_per_athlete, ymax = rev_per_athlete, 
                   color = group), 
               position = position_dodge(width = 0.8), show.legend = FALSE) + 
  geom_point(aes(x = sports, y = rev_per_athlete, color = group),
             position = position_dodge(width = 0.8), shape = 21) + 
  geom_point(aes(x = sports, y = exp_per_athlete, color = group),
