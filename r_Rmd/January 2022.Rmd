...
```{r include = TRUE, tidy = TRUE, out.width = '400px', fig.align = 'center'}
mtcars %>% 
...
```{r include = TRUE, tidy = TRUE, out.width = '300px', fig.align = 'center'}
mtcars %>% 
  ggplot(aes(mpg, hp, fill = as.factor(cyl))) + 
  geom_point(size = 6, shape = 21) + 
...
```{r model-summary, include = TRUE, comment = '#'}
...
.pull-left[
```{r tt-data, include = TRUE, warning = FALSE, message = FALSE}

ar_bees <- bees %>% 
  mutate(year = as.numeric(year)) %>% 
  group_by(year, state) %>% 
  summarize(lost_pct_mean = mean(colony_lost_pct, na.rm = TRUE), 
            reno_pct_mean = mean(colony_reno_pct, na.rm = TRUE)) %>% 
...
.pull-right[
```{r tt-plot, include = TRUE, echo = FALSE, warning = FALSE}
ar_bees %>% 
  ggplot() + 
  geom_point(aes(year, lost_pct_mean)) + 
  geom_point(aes(year, reno_pct_mean)) + 
