...
mls %>% 
  filter(round == "regular", Season == 1996) %>% 
  maketable_all() %>% str()
...
league_pos <- map(c(1996:2015), .f = mls_pos) %>% 
  reduce(full_join)
...
# 20220223: More Soccer Data with {gt}
...
s2016 %>%
  filter(home == "Sporting Kansas City" | visitor == "Sporting Kansas City") %>% 
  mutate(gf = case_when(home == "Sporting Kansas City" ~ hgoal,
                        visitor == "Sporting Kansas City" ~ vgoal),
         ga = case_when(home == "Sporting Kansas City" ~ vgoal,
                        visitor == "Sporting Kansas City" ~ hgoal),
