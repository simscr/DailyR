...
```{r d14-p1, eval = FALSE}
input <- advent_input(14)

start <- input$x[1]

rules <- input %>% 
  slice(-c(1:2)) %>% 
  separate(x, c("pair", "insert"), sep = " -> ") %>% 
  mutate(pair1 = paste0(str_sub(pair, 1, 1), insert),
         pair2 = paste0(insert, str_sub(pair, 2, 2)),
         all = paste0(str_sub(pair, 1, 1), insert, str_sub(pair, 2, 2)))

part1_all <- tibble(
  pair = paste0(start, c(lead(start)))[-length(start)]
) %>% 
  left_join(rules) %>% 
  pluck("pair1") %>% 
  paste0(collapse = "")

part1 <- paste0(part1_all$pair1, collapse = "")

polymer <- start
round <- 0


poly_fun <- function(polymer) {
  polymer <- polymer %>% 
    strsplit("") %>% 
    .[[1]]
  
  polymer <-  tibble(
    pair = paste0(polymer, c(lead(polymer)))[-length(polymer)]
  ) %>% 
    left_join(rules) %>% 
    pluck("pair1") %>% 
    paste0(collapse = "")
...
r11 %>% strsplit("") %>% .[[1]] %>% as_tibble() %>% count(value) %>% arrange(n)
...
# nchar(paste0(starting, c(lead(starting))))

# browseURL("https://twitter.com/drob/status/1470837319544487947/photo/1")
```
...
These challenges were difficult. I learned some new things like `lead`, `lag`,
`cumsum`, `scipen`, the utilty of user functions, and that trial-and-error only
goes so far. I hope to be better at this next year. Overall, it was a good
experience and it was interesting to see other people's solutions.