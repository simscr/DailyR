...
ar_cities_df <- ar_cities$osm_multipolygons %>% 
  ...
  mutate(long = st_coordinates(city_center))
  separate(city_center, into = c("lat", "long"), sep = ,, remove = FALSE)
  mutate(city_lat = city_center[1], 
         city_long = city_center[2])
...
ggplot(ar_map) + 
  geom_sf(fill = "#F8F8F8FF", color = "black") + 
  geom_sf(data = arnp_df, aes(geometry = geometry), fill = "#0072CEFF", color = "#0072CEFF") + 
  geom_sf(data = ar_cities_df, aes(geometry = city_center)) + 
  theme_void()
